...
真實表單（`砂量統計_NEW .xlsx`，工作表：可用數據分析）的欄位如下。
後端讀取時以中文名稱直接 mapping，**不要求對方改欄位名稱**。

| xlsx 欄位名稱  | 內部欄位名稱           | 說明                                         |
| ------------ | ------------------ | -------------------------------------------- |
| 項次           | `record_id`        | 資料列編號，可選                              |
| B開幅(%)       | `B_open_pct`       | B 進料口開幅，單位 %，允許超過 100%（同槽雙門同開） |
| C開幅(%)       | `C_open_pct`       | C 進料口開幅，單位 %，允許超過 100%           |
| D開幅(%)       | `D_open_pct`       | D 進料口開幅，單位 %，允許超過 100%           |
| R開幅(%)       | `R_open_pct`       | R 進料口開幅，單位 %，允許超過 100%           |
| 槽增加量(%)     | `tank_change_pct`  | 桶槽液位變化量，單位 %，可正可負               |
| 實際用量(kg)    | `usage_kg`         | 實際使用量，單位 kg；可為 0（該小時無使用）    |

### 上傳時的額外參數（不在 xlsx 內）

| 參數名稱           | 說明                              | 預設值   |
| ---------------- | --------------------------------- | ------- |
| `kg_per_percent` | 每 1% 液位變化對應公斤數             | 1120    |

此參數在前端上傳表單以輸入欄位提供，後端以此換算 `tank_change_kg`。

### 固定假設

* 每筆資料區間固定為 **1 小時**（`duration_minutes = 60`），不需欄位
* 開幅百分比在後端統一除以 100 轉為小數再進入模型（B_open = B_open_pct / 100）
* `usage_kg = 0` 為合法資料（該小時無使用，但進料口仍可能有進料進入桶槽），不排除

### 目標補充量計算

```text
tank_change_kg   = tank_change_pct * kg_per_percent
...
其中 `B_open = B_open_pct / 100`（其餘同），允許 > 1.0。
...
```text
進料口在開幅 100%（即小數 1.0）時，每小時的平均有效補料公斤數
```
